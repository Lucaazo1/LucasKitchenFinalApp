...
{
    public class LucasSaveData
    {
        private static HttpClient client = new HttpClient();
        
        [FunctionName("LucasSaveData")]
        public void Run([IoTHubTrigger("messages/events", Connection = "IotHubEndpoint")]EventData message,
            [CosmosDB(databaseName: "1234goodservername2.database.windows.net", collectionName: "Data", ConnectionStringSetting = "CosmosDB", CreateIfNotExists = true)] out dynamic output,
            ILogger log)
        {
            try
            {
